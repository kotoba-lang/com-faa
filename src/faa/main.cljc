(ns faa.main
  "Kotodama WASM entrypoint for the Faa clean-room actor (L4) — Clojure port.
  ...
  facts (`faa.<Entity>/<field>`); `*store*` is the in-memory materialization
...
(defn emit-facts
  "EAVT facts for one record: {\"faa.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "faa-compat" :tier tier :entities entities} 200])
